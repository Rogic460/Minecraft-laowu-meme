...
### 新增功能
- **耄耋猫多方块结构**：用丛林系方块（木板/楼梯/活板门/台阶）搭一个 6×6 立面（23 个方块，单 z 层），附近 10 格内一只命名为「耄耋」的猫会被召到结构锚点楼梯顶坐下（服务端权威扫描，每 20 tick 识别）。猫坐下后玩家 5 格内循环播放哈气音效（`maodie.ogg`，16 格内可闻、自然距离衰减）并转头盯最近玩家；破坏结构任一部件（尤其锚点楼梯）即解除、猫恢复自由。
...
### 调整
- **活板门朝向放宽**：结构匹配时忽略活板门朝向（facing 任意），仅卡开/关（open）与上半/下半（half）；楼梯朝向仍严格，以防其它楼梯误触发。
