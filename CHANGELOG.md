...
### 新增功能
- **耄耋猫多方块结构**：用任意木系方块（橡木/云杉/桦木/丛林/金合欢/深色橡木/樱桃木等，混搭也行；木板/楼梯/活板门/台阶）搭一个 6×6 立面（23 个方块，单 z 层），附近 10 格内一只命名为「耄耋」的猫会被召到结构锚点楼梯顶坐下（服务端权威扫描，每 20 tick 识别）。猫坐下后玩家 5 格内循环播放哈气音效（`maodie.ogg`，16 格内可闻、自然距离衰减）并转头盯最近玩家；破坏结构任一部件（尤其锚点楼梯）即解除、猫恢复自由。
- **结构支持 4 向旋转**：整体可旋转 90°/180°/270° 摆放，mod 自动识别所有朝向（楼梯朝随结构一起旋转比对），不再限定某个固定朝向。
- **命名换皮（哈基米贴图）**：任意猫命名为「耄耋」后底图换成对应其自身花色的「哈基米」风格贴图（纯客户端、按名字判定，不依赖结构）；幼猫保持原版贴图（避免报错贴图）。猫贴图来自 maocraft 材质包（作者一A化三哈，已内置全套家猫花色）。

### 修复（Bug）
- **贴图只在准星对着猫时变化**：旧逻辑靠 `state.nameTag` 判定名字，但 vanilla 仅在名字可见（准星对着 / `shouldShowName()` 为真）时填充 `nameTag`，准星离开即失效。改为在 `extractRenderState` 里直接读真实体 `getCustomName()`，判定与贴图路径解析都放在这里，准星移开也保持换皮。
- **幼猫命名后黑紫报错贴图**：原方案对婴儿猫剥离 `_baby` 后缀走成猫图，不稳定导致黑紫。改为幼猫不启用换皮、直接走原版贴图。

### 调整
- **木种不限**：结构匹配只认方块**类型**（楼梯就是楼梯、活板门就是活板门），不限制具体木种，不同位置可混搭。
- **活板门朝向放宽**：结构匹配时忽略活板门朝向（facing 任意），仅卡开/关（open）与上半/下半（half）；楼梯朝向随结构旋转一起严格比对，以防其它楼梯误触发。
- **锚点 / 座位改为楼梯 `[4,1,0]`**：原结构需黑蜡烛做锚点、猫坐活板门上；现删蜡烛，锚点与猫座位都改到楼梯 `[4,1,0]`，猫 teleport 到其上方一格坐楼梯顶。
- 哈气音效范围从 3 格扩到 5 格。
- README 新增结构实拍图（`docs/jiegoutu.png`）与可旋转搭建说明。

### 致谢
- 猫贴图来自 maocraft 材质包（作者一A化三哈，B 站 https://space.bilibili.com/1756604526），已获授权内置。
