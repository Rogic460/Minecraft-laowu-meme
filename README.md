...
除了「老吴猫」，本 mod 还有一套**耄耋猫**玩法：用丛林系方块搭一个立面结构，附近一只命名为「耄耋」的猫会被召到结构座位上；猫坐下后，玩家 5 格内持续循环播放哈气音效、并转头看向最近的玩家；破坏结构即解除。
...
这是一面「猫窝墙」：在地面上沿水平方向铺一个 **6 宽（x:0~5）× 6 高（y:0~5）** 的立面，所有方块都在**同一 z 层**（厚度 1 格）。全部用丛林系方块（丛林木板 / 楼梯 / 活板门 / 台阶）。
...
- **楼梯朝向（facing）需严格一致**（如 `[4,1,0]` 必须为朝西）——这是防止其它楼梯误触发的关键。
